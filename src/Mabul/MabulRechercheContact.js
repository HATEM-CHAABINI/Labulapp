...
const MabulRechercheContact = props => {
  ...
  const saveData = async data => {
    ...
    if (data.serviceType.name === 'alerts' && mabulService === 'Alerte') {
      firestore()
        ...
        .collection(data.serviceType.name)
        .add(data)
        .then(async res => {
          const responce = firestore()
            .collection('userAlerts')
            .doc(auth().currentUser.uid)
            .collection(data.serviceType.name)
            .doc(res.id);
  ...
  const onSubmit = () => {
    ...
    if (props.shareEdit == true) {
      ...
      if (mabulService === 'organize') {
        data = Object.assign(demandData, {
          contactType: contactType,
          users: selectedUser,
          serviceType: {name: 'organize', code: 0, subCode: 0},
          status: {status: 'INPROGRESS', code: 102},
        });
      } else if (mabulService === 'give') {
        data = Object.assign(demandData, {
          contactType: contactType,
          users: selectedUser,
          serviceType: {name: 'give', code: 1, subCode: 0},
          status: {status: 'INPROGRESS', code: 102},
        });
      } else if (mabulService === 'sell') {
        data = Object.assign(demandData, {
          contactType: contactType,
          users: selectedUser,
          serviceType: {name: 'sell', code: 2, subCode: 40},
          status: {status: 'INPROGRESS', code: 102},
        });
      } else if (mabulService === 'need') {
        data = Object.assign(demandData, {
          contactType: contactType,
          users: selectedUser,
          serviceType: {name: 'need', code: 3, subCode: 11},
          status: {status: 'INPROGRESS', code: 102},
        });
      }

      if (mabulService === 'Alerte') {
        data = Object.assign(alertData, {
          contactType: contactType,
          users: selectedUser,
          serviceType: {name: 'alerts', code: 0, subCode: 0},
          status: {status: 'INPROGRESS', code: 102},
        });
    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <Text
          ...
          Choisis un ou plusieurs cercles, ou recherche un contact précis.
        </Text>
        ...
        <View
          ...
          <ShareButton
            ...
            style={{
              marginBottom: 10 * hm,
              backgroundColor: 'rgba(215,243,255,0.7)',
            }}
            leftIcon={<Voisins />}
            rightIcon={
              <CheckBox
          ...
          <ShareButton
            ...
            style={{
              marginBottom: 10 * hm,
              backgroundColor: 'rgba(255,244,217,0.62)',
            }}
            leftIcon={<Amis />}
            rightIcon={
              <CheckBox
          ...
          <ShareButton
            ...
            style={{
              marginBottom: 10 * hm,
              backgroundColor: 'rgba(255,23,103,0.11)',
            }}
            leftIcon={<Famille />}
            rightIcon={
              <CheckBox
          ...
          <ShareButton
            ...
            style={{
              marginBottom: 10 * hm,
              backgroundColor: 'rgba(240,245,247,0.62)',
            }}
            leftIcon={<Tous />}
            rightIcon={
              <CheckBox
      ...
      <RBSheet
        ...
        <OkModal
          hideDescription={() => {}}
          closeModal={() => Sheet1.current.close()}
        />
      </RBSheet>
    </View>
  );
