...
export const fetchcoordinate = async (latitude, longitude) => {
  let RefNeed = firestore()
    .collectionGroup('need')
    .where('coordinate.latitude', '==', latitude)
    .where('coordinate.logitude', '==', longitude);
  let allNeeds = await RefNeed.get();
  if (!allNeeds.empty) {
    return true;
  }
  let Reforganize = firestore()
    .collectionGroup('organize')
    .where('coordinate.latitude', '==', latitude)
    .where('coordinate.logitude', '==', longitude);
  ...
  let Refgive = firestore()
    .collectionGroup('give')
    .where('coordinate.latitude', '==', latitude)
    .where('coordinate.logitude', '==', longitude);
...
export const fetchDemandFiltre = async (filtre, filtreD) => {
  console.log(
    'waaaaaaaaa',
    filtreD != 'Toutes' && filtreD.length != 0 && filtreD !== undefined,
  );
  ...
  for (const elem of filtre) {
    if (elem == 'vends') {
      ...
      if (filtreD != 'Toutes' && filtreD.length != 0 && filtreD !== undefined) {
        RefSell = firestore()
          .collectionGroup('sell')
          .where('demandStartDate', '>', d1)
          .where('demandStartDate', '<', d2);
      } else {
      ...
      for (const doc of allSell.docs) {
        if (doc.data().coordinate.latitude !== undefined) {
          console.log(
            '1. ',
            doc.data().demandStartDate.seconds,
            '2. ',
            d1,
            ' 3. ',
            d2,
...
export const fetchDemands = async () => {
  ...
  const responseOrganize = firestore()
    .collection('userDemands')
    .doc(auth().currentUser.uid)
    .collection('organize');
...
export const createUser = async signupData => {
  ...
  return auth()
    ...
    .then(userCredential => {
      var userCreated = userCredential.user;
      let {firstName, lastName, email, activeLocation, activeNotification} =
        signupData;
      if (userCreated) {
        let DataToBeSet = {
          firstName: firstName,
