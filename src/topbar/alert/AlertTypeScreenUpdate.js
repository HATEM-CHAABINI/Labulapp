...
const AlertTypeScreenUpdate = (props) => {
  const conceptColor = '#F9547B';
  const alertData = useSelector((state) => state.alertReducer);
  const dispatch = useDispatch()
  const initialValues = {
    alertDescription: props.item.type.title,
  };
  ...
  const onSubmit = (values) => {
    dispatch(update_into_alert({ alertType: values,data:props.item }))
    Actions.pop();
  ...
  console.log(props.item.type.title)

  return (

    <View style={styles.container}>
      <MabulCommonHeader noBackButton={"no"} style={styles.header} onRightPress={()=>Actions.pop()} percent={props.process} progressBarColor={conceptColor} />
...
export default AlertTypeScreenUpdate;
